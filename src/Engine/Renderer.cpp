...
void Renderer::FillBuffers(std::unique_ptr<Voxel>& voxel){
  ...
  for (int i = 0; i < 108; i+=3) {
    std::cout << fmt::format("colour: {} | {} | {}\n",  voxel->colour_.r,  voxel->colour_.g, voxel->colour_.b);
...
void Renderer::AddVoxel(std::unique_ptr<Voxel>& voxel){
...
void Renderer::Render() {
  ...
  if (gui_.FlagSet(Interface::Flags::kShow)) {
    ...
    for (const auto& v : voxels_) {
      ...
      std::cout << "size: " << v->indices_.size() << std::endl;
