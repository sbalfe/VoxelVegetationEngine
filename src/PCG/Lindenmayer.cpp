...
void Lindenmayer::ProcessString(uint32_t chunk_index) {
  ...
  turtle->state_.chunk_voxel_position_ = Position{chunk->GetSize() / 2.0,
                                        0,
                                                 chunk->GetSize() / 2.0};

  turtle->state_.direction_ = Position{0.0,1.0,0.0};
  ...
  std::for_each(std::begin(state->result_), std::end(state->result_), [&](char c){
    ...
    if (isdigit(c)) {
      ...
      if (c == '>'){
        Position initial_position = turtle->state_.chunk_voxel_position_;
        ...
        for (int i = 0; i < state->branch_length_ ; i++){
          ...
          Position current_voxel = turtle->state_.chunk_voxel_position_.Floor(1);
...
uint32_t Lindenmayer::AddScene(Chunk* chunk, State* state){
  scenes_[chunk_tag_counter_] = new Scene{chunk, state, new Turtle{new std::stack<TurtleState>,{}}};
  return chunk_tag_counter_++;
}

Lindenmayer::State* Lindenmayer::CreateState(std::string axiom, double branching_angle, uint32_t branch_length){
  return new State(std::move(axiom), "", branch_length, branching_angle);
}
...
//void Lindenmayer::PlaceCube(Position& cube_center, uint32_t cube_size){
